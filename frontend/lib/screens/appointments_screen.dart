...
/// A single live booking the patient currently holds.
class _Booking {
  final String token;
  final String doctor;
...
class _AppointmentsScreenState extends State<AppointmentsScreen> {
  // Start with an active booking so the booked-state is visible. Cancelling
  // sets this to null, which flips the page to the "book now" empty state.
  _Booking? _booking = _Booking(
    token: '047',
    doctor: 'Dr. Chameli',
    specialty: 'General Medicine',
    hospital: 'Chameli Hospital',
    date: DateTime.now(),
    time: '10:30 AM',
    estWait: '~1 hour',
  );
  ...
  Future<void> _bookNow() async {
    ...
    // Demo: returning from the booking flow restores a confirmed booking.
    if (!mounted) return;
    setState(() {
      _booking ??= _Booking(
        token: (40 + DateTime.now().second % 30).toString().padLeft(3, '0'),
        doctor: 'Dr. Chameli',
        specialty: 'General Medicine',
        hospital: 'Chameli Hospital',
        date: DateTime.now(),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: _booking == null ? _buildEmpty() : _buildBooked(_booking!),
    );
  }
              ...
              children: [
                ...
                PrimaryButton(
                  ...
                  onTap: () {
                    ...
                    _snack('Appointment rescheduled successfully.');
                  },
                ),
              ],
            ),
                ...
                Expanded(
                  child: ElevatedButton(
                    onPressed: () {
                      ...
                      _snack('Booking cancelled.', error: true);
                    },
