...
export default function Queue() {
  ...
  const markServed = async (booking) => {
    await update(COLLECTIONS.BOOKINGS, booking.id, {
      status: BOOKING_STATUS.SERVED,
      servedAt: new Date().toISOString(),
    })
    toast.success(`Token #${booking.tokenNumber} marked served`)
  }

  const markNoShow = async (booking) => {
    if (!confirm(`Mark token #${booking.tokenNumber} as no-show?`)) return
    await update(COLLECTIONS.BOOKINGS, booking.id, {
      status: BOOKING_STATUS.NO_SHOW,
    })
    toast.success(`Token #${booking.tokenNumber} marked as no-show`)
  }
